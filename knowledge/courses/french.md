---
title: French Language Coaching
category: Language training and TEF/TCF preparation
source: data/courses/french.json, data/pricing.json
status: verified
last_reviewed: 2026-08-25
availability: active
pricing_status: published
demo_available: null
pricing:
  - pack_id: french-basic-to-tef-usd
    name: French Basic to TEF USD
    price: 400
    original_price: null
    discount: null
    currency: USD
    duration: "64 weeks"
  - pack_id: french-basic-a1-evening
    name: French Basic & A1 - Evening
    price: 7500
    original_price: null
    discount: null
    currency: INR
    duration: "150 days / 10 weeks"
  - pack_id: french-basic-a1-morning
    name: French Basic & A1 - Morning
    price: 7500
    original_price: null
    discount: null
    currency: INR
    duration: "150 days / 10 weeks"
  - pack_id: french-a2
    name: French A2
    price: 9750
    original_price: null
    discount: null
    currency: INR
    duration: "90 days / 7 weeks"
  - pack_id: french-b1
    name: French B1
    price: 9750
    original_price: null
    discount: null
    currency: INR
    duration: "90 days / 7 weeks"
  - pack_id: french-b2
    name: French B2
    price: 9750
    original_price: null
    discount: null
    currency: INR
    duration: "90 days / 7 weeks"
  - pack_id: french-basic-a1-all-timings
    name: French Basic & A1 - All Timings
    price: 9750
    original_price: null
    discount: null
    currency: INR
    duration: "150 days / 10 weeks"
  - pack_id: french-basic-a1-night
    name: French Basic & A1 - Night
    price: 10000
    original_price: null
    discount: null
    currency: INR
    duration: "150 days / 10 weeks"
  - pack_id: french-basic-a1-a2-evening
    name: French Basic, A1 & A2 - Evening
    price: 12000
    original_price: null
    discount: null
    currency: INR
    duration: "224 days / 16 weeks"
  - pack_id: french-basic-a1-a2-morning
    name: French Basic, A1 & A2 - Morning
    price: 12000
    original_price: null
    discount: null
    currency: INR
    duration: "224 days / 16 weeks"
  - pack_id: french-basic-a1-weekend
    name: French Basic & A1 - Weekend
    price: 12000
    original_price: null
    discount: null
    currency: INR
    duration: "150 days"
  - pack_id: french-basic-a1-a2-all-timings
    name: French Basic, A1 & A2 - All Timings
    price: 14250
    original_price: null
    discount: null
    currency: INR
    duration: "224 days / 16 weeks"
  - pack_id: french-basic-a1-a2-night
    name: French Basic, A1 & A2 - Night
    price: 14500
    original_price: null
    discount: null
    currency: INR
    duration: "224 days / 16 weeks"
  - pack_id: french-b1-b2-tef
    name: French B1, B2 & TEF
    price: 22125
    original_price: null
    discount: null
    currency: INR
    duration: "224 days / 16 weeks"
  - pack_id: french-basic-to-tef
    name: French Basic to TEF
    price: 34375
    original_price: null
    discount: null
    currency: INR
    duration: "64 weeks"
demo_schedule: null
batch_schedule:
  - day: Monday-Friday
    start: "6:30 AM"
    end: "8:00 AM"
    timezone: IST
    label: Basic Slot 1
  - day: Monday-Friday
    start: "4:30 PM"
    end: "6:00 PM"
    timezone: IST
    label: Basic Slot 2
  - day: Monday-Friday
    start: "8:00 AM"
    end: "9:30 AM"
    timezone: IST
    label: A1 Slot 1
  - day: Monday-Friday
    start: "6:00 PM"
    end: "7:30 PM"
    timezone: IST
    label: A1 Slot 2
  - day: Monday-Friday
    start: "8:00 AM"
    end: "9:30 AM"
    timezone: IST
    label: A2 Slot 1
  - day: Monday-Friday
    start: "6:00 PM"
    end: "7:30 PM"
    timezone: IST
    label: A2 Slot 2
  - day: Monday-Friday
    start: "6:30 AM"
    end: "8:00 AM"
    timezone: IST
    label: B1 Slot 1
  - day: Monday-Friday
    start: "7:30 PM"
    end: "9:00 PM"
    timezone: IST
    label: B1 Slot 2
  - day: Monday-Friday
    start: "4:30 PM"
    end: "6:00 PM"
    timezone: IST
    label: B2 Slot 2
  - day: Monday-Friday
    start: "8:00 AM"
    end: "9:30 AM"
    timezone: IST
    label: TEF/TCF Slot 1
demo_timings: null
batch_timings: "Basic 6:30-8:00 AM / 4:30-6:00 PM, A1 8:00-9:30 AM / 6:00-7:30 PM, A2 8:00-9:30 AM / 6:00-7:30 PM, B1 6:30-8:00 AM / 7:30-9:00 PM, B2 4:30-6:00 PM, TEF/TCF 8:00-9:30 AM"
aliases: [French, french language, tef, tcf, delf]
tags: [language, france, canada pr, francophone]
---

# French Language Coaching

## Overview

- **Best for:** Canada PR, TEF Canada, TCF Canada, DELF, Francophone study and career pathways
- **Levels:** Basic, A1, A2, B1, B2, TEF, TCF

## Eligibility

Open to all students. No prerequisite qualifications required for enrolment.

## Course Details

- **Features:** Live 90-minute sessions, C1-certified trainers, PDF reference material, vocabulary quizzes, weekly tests, performance tracking, speaking practice with real-time feedback, session recordings
- **Learning hours:** 120+ hours total learning in Basic-to-TEF pathway
- **Exam focus:** Aligned with TEF Canada and TCF Canada patterns for NCLC 7+ goals

## Fees

See `data/pricing.json` under `prices.french` for current pack prices and durations.

## Timings

| Batch | Slot 1 (IST) | Slot 2 (IST) |
|-------|--------------|--------------|
| Basic | 6:30 AM - 8:00 AM | 4:30 PM - 6:00 PM |
| A1 | 8:00 AM - 9:30 AM | 6:00 PM - 7:30 PM |
| A2 | 8:00 AM - 9:30 AM | 6:00 PM - 7:30 PM |
| B1 | 6:30 AM - 8:00 AM | 7:30 PM - 9:00 PM |
| B2 | -- | 4:30 PM - 6:00 PM |
| TEF/TCF | 8:00 AM - 9:30 AM | -- |

## FAQs

No French-specific FAQs documented in source material.

## Important Notes

- French proficiency can add up to 50 CRS points for eligible Canada Express Entry profiles when requirements are met.

## Status

Active course with full pricing and schedule available.

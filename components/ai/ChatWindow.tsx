...
//
// ⚠️ POSITIONING: stacked above ClientWrapper's LeadModal floating
// button (which sits at bottom-6/8 right-5/8, z-[9999]) — see the
// matching note in ChatWidget.tsx. Do not move this back to
// bottom-6 right-6 without also checking LeadModal's position.
...
const [conversationId, setConversationId] = useState<string | null>(null);
  ...
  return (
    <div
      className="fixed bottom-24 right-5 z-[9990] flex w-[92vw] max-w-sm flex-col overflow-hidden shadow-2xl sm:bottom-28 sm:right-8 sm:w-96"
      style={{
        borderRadius: CHAT_COLOURS.radius,
        // Bottom offset increased from 96px to 160px to match the
        // panel's new elevated position (bottom-24/28 instead of
        // bottom-6) — keeps the panel from running off the top edge
        // on short mobile viewports.
        maxHeight: "min(640px, calc(100dvh - 160px))",
        background: "#fff",
      }}
      role="dialog"
